...
def minibatch_parse(sentences, model, batch_size):
    ...
    while len(unfinished_parses) > 0:
        batch_unfinished_parses = unfinished_parses[:batch_size]

        transitions = model.predict(batch_unfinished_parses)
        for idx in range(len(batch_unfinished_parses)):
            batch_unfinished_parses[idx].parse_step(transitions[idx])
